...
namespace DeviareLiteInterop
{
    public sealed class HookLib
    {
        ...
        {
            //构造函数
            if(classType.Name == methodName)
            {
                ConstructorInfo constructorInfo = classType.GetConstructor(parameters);

                if (constructorInfo == null)
                    return IntPtr.Zero;

                System.Runtime.CompilerServices.RuntimeHelpers.PrepareMethod(constructorInfo.MethodHandle);
